...
def get_countries():
    ...
    if action == 'age':
        ...
        if not age_input or age_input not in ['16', '18']:
        ...
        if not user_age or int(user_age) < 16:
            messagebox.showerror("Error", "Invalid age. Please enter your age (minimum 16) for driving.")
            return

        driving_age = 18 if int(user_age) >= 18 else 16
        cur.execute("SELECT country_name FROM countries WHERE age <= ?", (driving_age,))
    else:
        messagebox.showerror("Error", "Invalid action. Please enter 'drive'.")
        return
